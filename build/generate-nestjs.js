...
function generateNestjsServiceController(ctx, fileDesc, sourceInfo, serviceDesc) {
    ...
    const t = options.useContext ? `<${main_1.contextTypeVar}>` : '';
  ...
  `);
    serviceDesc.method.forEach((methodDesc, index) => {
        ...
        if (options.addGrpcMetadata) {
            ...
            params.push(ts_poet_1.code `metadata${q}: ${ts_poet_1.imp('Metadata@grpc')}`);
        }
      ...
      `;
        ...
        chunks.push(ts_poet_1.code `
      ${name}(${ts_poet_1.joinCode(params, { on: ', ' })}): ${returns};
    `);
        if (options.useContext) {
